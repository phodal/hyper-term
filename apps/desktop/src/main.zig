...
const max_agent_operation_id_bytes = agent_block_view.max_operation_id_bytes;
...
pub const Model = struct {
    ...
    pub fn hasAgentSearchQuery(model: *const Model) bool {
        return DesktopView.agentSearchQuery(model).len > 0;
    }

    pub fn agentSearchResultCount(model: *const Model) usize {
        return DesktopView.agentSearchMatchCount(model);
    }
